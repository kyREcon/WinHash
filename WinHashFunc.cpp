...
Cleanup:
	...
	if (bSuccess)
	{
		...
		pHashInfo->pHashDataString = HashInfoTmp.pHashDataString;
		pHashInfo->StringHashLength = HashInfoTmp.StringHashLength;
		pHashInfo->pHashDataRaw = HashInfoTmp.pHashDataRaw;
		pHashInfo->RawHashSize = HashInfoTmp.RawHashSize;
	}
	else
	{	
		WinHashDeleteHash(&HashInfoTmp);
	...
	if (HashStringTmp)
	{
		SecureZeroMemory(HashStringTmp, StringHashLength);
		delete[] HashStringTmp;
	}
