...
int main()
{
	...
	if (WinHashInitHashInfoStruct(MD5, &HashInfo))
	{
		...
		if (WinHashInitReHashInfoStruct(SHA256, &HashInfo)) //if this fails, we must call WinHashDeleteHash to free memory allocated from WinHashGetHash
		{
			if (WinHashReHash(&HashInfo, 1)) //if WinHashReHash fails we don't need to call WinHashDeleteHash
			{
